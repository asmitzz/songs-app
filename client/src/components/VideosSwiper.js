...
const VideosSwiper = ({ title, pathname, videosArray, history }) => {
  ...
  const checkInPL = (id) => selectedPlaylist && selectedPlaylist.playlist.find((v) => v.id === id);
  ...
  return (
    <div>
      <div className="landing__page__headings">
        <h2>{title}</h2>
        <button className="seeall__btn" onClick={() => history.push(pathname, { title })}>
          see all
        </button>
      </div>
        ...
        { videosArray.map((video) => (
          <SwiperSlide key={video.id}>
            ...
            <div className="card__footer">
              <button className="primary-btn" onClick={() => AddToWatchLater(video)} >
                <i className="fas fa-clock" aria-hidden="true"></i>
                { watchLater.find((v) => v.id === video.id) ? " Remove" : " Watch later"}
              </button>

              <div>
                <button className="secondary-btn" disabled={checkInPL(video.id)} onClick={() => AddToPlaylist(video)}>
                  <i className="fa fa-music" aria-hidden="true"></i>
                  {checkInPL(video.id) ? "Added to" : "Add to"}
                </button>
                
                <select
                  className="playlist__selector primary-btn"
                  value={selectedPlaylist ? selectedPlaylist.id : "none"}
                  onChange={(e) =>
                    dispatch({
